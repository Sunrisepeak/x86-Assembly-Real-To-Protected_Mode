...
read_hard_disk_0:                           ; //read a logic-sector
                                            ; input(parameter):
         pushad                             ;       sentor : push-4
                                            ;       M-area-select: push-4
;        stack:                             ;       offset: push-4
;              ......                       ; return EBX(+ 512)
;              ......
;              [ssOfCaller]                 ; Exist while call by gate and change stack
;              [espOfCaller]                
...
;              pushad(universal reg 8)  <--- esp

         mov ebp, esp                                           
         mov esi, 8                             ; offset for stack-default-parameter
         
         mov ax, [ebp + 9 * 4]                  ; get code-seg-selector of caller
         arpl [ebp + 8 * 4 + esi + 1 * 4], ax   ; adjust rpl of target area
         ...
         inc dx                                 ;0x1f3
         mov eax, [ebp + 8 * 4 + esi + 2 * 4]   ; get sector
         out dx,al                              ;LBA地址7~0
  ...
  .waits:
         ...
         mov dx,0x1f0
         mov ds, [ebp + 8 * 4 + esi + 1 * 4]    ; get ds of target area
         mov ebx, [ebp + 8 * 4 + esi]           ; get offset of target area
  ...
  .readw:
         ...
         mov [ebp + 8 * 4 + esi + 2 * 4], ebx   ; save ebx( + 512)
         
         popad
         
         push ebp
         mov ebp, esp
         mov ebx, [ebp + 4 * 4 + 4]             ; return ebx
         pop ebp
         ; imm necessary is equality with parameter-size, otherwise gate retrun exception
         retf 12                                ; seg return and pop parameter of all
...
;-----------------------------Ex02 start---------------------------------------         
         ...
         push dword [ebp + 12 * 4]          ; get sector from stack
         push dword core_data_seg_sel       ; get select of target buffer
         push dword core_buf                ; get offset of buffer

         call sys_routine_seg_sel:read_hard_disk_0  ; return ebx
...
;-----------------------------Ex02 start---------------------------------------         
         ...
         mov eax, [ebp + 12 * 4]            ; set sector -> eax
         
    .b1:
         push eax                           ; set sector
         push dword mem_0_4_gb_seg_sel      ; set select of target area
         push dword ebx                     ; set begin address
...
;-----------------------------Ex02 end-----------------------------------------
         ...
         mov ecx,[es:edi+0x2c]              ;U-SALT条目数(通过访问4GB段取得) 
         add edi,0x30                       ;U-SALT在4GB段内的偏移 
  .b2: 
         push ecx
         push edi
...
;-----------------------------Ex02 start-------------------------------------
         ...
         jnz .No3                           ;
         mov cx, 1_11_0_1100_000_00011B     ; while ecx = 3,(readdisk-gate set)
                                            ; 3pl-access and 3-parameter
  .No3:
